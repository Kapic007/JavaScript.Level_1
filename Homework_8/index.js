...
// функция добавления в корзину (с проверкой превышения наличия на складе)
function addInBasket() {
  let itemIdx;
  if (
    !basket.some((el, index) => {
      if (el.productId === this.id) itemIdx = index;
    ...
    basket[itemIdx].amount + 1 < this.warehouse
      ? (basket[itemIdx].amount += 1)
      : (basket[itemIdx].amount = this.warehouse);
  }
}

/* в каждый товар (объект товара) добавляю функцию подсчёта цены
и функцию добавления в корзину */
for (let key in products) {
  products[key].getPrice = gotPrice;
  products[key].addToBasket = addInBasket;
}
...
function basketShow() {
  ...
  if (basket.length) {
    let amountSum = basket.reduce((acc, item) => acc + item.amount, 0);
    basketBlock.innerText = `В корзине: ${amountSum} товаров на сумму ${countBasketPrice()} рублей`;
...
function catalogShow(filterValue) {
  ...
  if (catalogOn) {
    selectorAdd(filterValue); //добавляем ползунок фильтра
    addFilterListener(); //добавляем обработчик на ползунок фильтра

    if (products) {
      //создаем "шапку" для таблицы товаров
      for (let i = 0; i < catalogHeaderNames.length; i++) {
        ...
        for (let key in products) {
          //если товар подпадает под фильтр -- переходим к следующему товару
          if (filter(key)) {
            continue;
          }
  ...
  for (let i = 0; i < basketHeaderNames.length; i++) {
    ...
    for (let item of basket) {
      ...
      if (products[item.productId].hasOwnProperty(basketHeaderNames[i])) {
        ...
        addPlusMinusButtons(item, basketItem, basketHeader);
...
// фнкция добавления кнопок увеличения/уменьшения кол-ва товаров
function addPlusMinusButtons(item, basketItem, basketHeader) {
...
function selectorAdd(filterValue) {
  ...
  selectorForm.setAttribute("oninput", "price.value=parseInt(filter.value)");
  selectorForm.innerHTML = `<label for="filter">Максимальная цена:</label>
  <input type="range" id="filter" name="filter" value="${
    filterValue ?? maxPrice
  }" min="0" max="${maxPrice}">${maxPrice}<br><br><output name="price" for="filter"></output>`;
  catalog.appendChild(selectorForm);
}

//обработчик ползунок-фильтр
function addFilterListener() {
  let filterPrice = document.getElementById("filter");
...
//функция-фильтр
function filter(key) {
  return products[key].price > document.getElementById("filter").value;
}
